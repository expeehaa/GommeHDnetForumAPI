    ...
    public class Program
    {
        ...
        private static void TestForumThreads(Forum forum)
        {
            ...
            if (someForum != null)
            {
                ...
                Console.WriteLine(string.Join(", ", t1.Messages.Select(m => $"({m.ToString()})")));
            }
            else Console.WriteLine($"No forum with name '{forumname}' found\n");
        ...
        public static void AllUserAccountsBeforeId(Forum forum, int maxId)
        {
            ...
            for (var i = 1; i < maxId; i++)
            {
                ...
                try
                {
                    ...
                    else if (!ui.Verified.HasValue || !ui.Verified.Value) text = "not verified";
                    if (ui?.Verified != null && ui.Verified.Value) userinfos.Add(ui);
                }
                catch (Exception e)
                {
